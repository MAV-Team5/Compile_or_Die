...
| `IAnimationDriver` | 모션 이름 | `AnimatorDriver` |

**모션은 연출 묶음에 들어있다.** 증강은 **파라미터 이름과 값만** 넘기고, 어떤 모션이 나가고 언제 돌아올지는 전부 Animator 가 정한다.

```
castFx     Motion Parameter "Attack"  Value 1     시전자가 공격
launchFx   Motion Parameter "Fire"    Value 1     터렛이 발사
hitFx      Motion Parameter "Hurt"    Value 1     맞은 적이 피격
tickFx     Motion Parameter "Burning" Value 1     타는 상태 켜기 (Bool)
                            "Burning" Value 0     끄기
```

| Animator 파라미터 타입 | 값의 뜻 |
|---|---|
| Trigger | 1이면 켜고, 0이면 도로 내린다 |
| Bool | 0이면 false, 아니면 true |
| Int · Float | 그대로 |

**상태 이름을 직접 재생하지 않는 것이 핵심이다.**

```
상태 이름   Animator 의 내부 구현.  이름을 바꾸면 증강이 깨진다
파라미터    Animator 가 밖에 내주는 공개 통로
```

`CrossFade` 로 상태를 직접 틀면 전이를 건너뛰어 블렌딩·레이어가 어긋나고, 무엇보다 **"언제 돌아오나"의 규칙이 코드와 Animator 두 곳으로 갈린다.** 파라미터만 넘기면 그 판단이 Animator 한 곳에만 있어서, 애니메이션 담당은 Animator 창만 보면 된다.

별도 Effect 모듈로 만들지 않은 이유 — `FxGroup` 은 이미 **타이밍**(시전·발사·적중·틱)과 **대상**을 알고 있다. Effect 로 만들면 "적중 시" 하나만 커버하고 시전·발사 타이밍은 못 잡는다.

`AnimatorDriver.mappings` 는 오브젝트마다 파라미터 이름이 다를 때만 채운다 — **증강 에셋은 안 고치고** 터렛은 `Fire → Shoot_01`, 드론은 `Fire → Laser` 로 갈아끼운다.
...
```csharp
// AugmentRunner.Tick
ctx.Begin(transform, Instance);   // 원점이 자기 transform
```

**`AugmentRunner` 는 이미 소환물이다.** 플레이어 밑에 있으면 플레이어가 원점이고, 월드에 떼어놓으면 그 자리가 원점이 된다. 그래서 소환은 새 축이 아니라 *러너를 하나 더 놓는 효과*다.

```
SummonEffect   적중 자리에 프리팹을 놓고 러너를 붙인다
Summon         수명과 개수 상한.  증강별로 몇 마리 살아있는지 센다
AugmentRunner  selfDriven 을 켜면 관리자 없이 스스로 돈다
```

**소환물의 공격은 평범한 증강 에셋이다.**
...
Java (소환 증강)
   ...
   Delivery   Instant
   Effect     Summon
                summonPrefab   JVM 터렛 몸통
                behaviour      "JVM 사격"  ← 이것도 증강 에셋
                duration       비워둠 (시트 지속시간)
                maxAlive       0 (시트 수량)

JVM 사격 (소환물이 실행할 증강)
   Trigger    Cooldown
   Targeting  Nearest          ← 터렛 자리 기준으로 검색된다
   Delivery   Projectile
   Effect     Damage
```
...
| | |
|---|---|
| `PlayerStats` 자동 적용 | `AugmentInstance.Stat` 을 거치므로 하드웨어 업그레이드가 소환물에도 먹는다 |
| 레벨 승계 | 소환한 증강의 레벨을 그대로 쓴다. 같이 성장한다 |
| 타겟팅이 소환물 기준 | `ctx.Owner` 가 소환물이라 "터렛 주변 적"이 공짜 |

> **자기 자신을 소환하는 조립은 막아뒀다.** `behaviour` 가 소환 증강 자신이면 경고를 띄우고 건너뛴다 — 안 막으면 한 프레임에 화면이 덮인다.
>
> 소환물이 *다른* 소환 증강을 부르는 것은 막지 않았다. 상한(`maxAlive`)이 있어서 폭주하지는 않지만, 조립할 때 의도한 것인지 확인할 것.
...
**숫자 하나로 트리부터 그물까지 연속으로 갈린다.**

| `neighborsPerNode` | 모양 | 간선 수 |
|---|---|---|
| 1 | **Tree** — 가장 가까운 하나에만. 사이클 없음 | 노드 − 1 |
| 2~3 | **그물** — 무리 모양을 따라 둥글게 이어진다 | 노드 × 이웃수 |
| 0 | **완전 그래프** — 거리 안 전부 | 노드 × (노드−1) ÷ 2 |

**`linkRange` 를 반드시 걸 것.** 거리 제한이 없으면 화면 반대편 노드끼리도 이어져 **플레이어 위를 가로지르는 긴 간선**이 생긴다. 거리를 걸면 뭉친 무리를 따라 국소적으로 이어져 덩어리가 눈에 들어온다.

```
거리 없음        노드 10개  →  45줄이 화면을 가로지른다
거리 + 이웃 2    →  20줄이 무리 모양을 따라간다
```

**부모를 어디서 고르느냐가 두 계열을 가른다.**

| `parentMode` | 부모 | 성격 |
|---|---|---|
| `Nearest` | 이번 발동에서 놓인 노드 중 가장 가까운 것 | **평면적** — 한 번에 이어붙이고 끝 |
| `Origin` | 이 파이프라인의 원점 = 직전에 맞은 적 | **누적적** — 기존 구조 위에 층이 쌓인다 |

`Origin` 은 하위 파이프라인 안에서만 의미가 있다. `BeginChild` 가 원점을 적중한 적으로 잡아주기 때문에, **"투사체가 꿴 적이 부모, 그 너머 부채꼴의 적들이 자식"** 이 조립만으로 나온다.

**트리는 발동마다 한 층씩 자란다.**

```
1발차   A 적중  →  A의 자식 B, C                 깊이 1
2발차   다시 적중  →  B, C 는 수명만 갱신  →  그 뒤에 D, E, F, G      깊이 2
3발차   →  깊이 3
```

그러려면 **어디가 잎인지** 알아야 한다. 그래서 간선에 방향(`ToChild`)이 있고, `TreeFrontier` 가 자식 간선을 타고 끝까지 내려가 잎만 골라낸다. 도중의 노드는 건드리지 않는다.

```
Targeting   Nearest
Delivery    Projectile
Effect      SubPipeline                       ← 잎 찾기
              targeting  TreeFrontier
                 maxDepthOverride  0          (시트 깊이 = 최대 층수)
              delivery   Instant
              effect     SubPipeline          ← 잎에서 자라기
                           targeting  AllInRange
                              halfAngle    35~45
                              targetLimit  2   ← 갈라지는 가지 수
                           delivery   Instant
                           effect     Link  parentMode = Origin
```

**`Chain` 을 쓰면 안 된다.** `Chain` 은 한 발차에 `depth` 층을 한꺼번에 만들어서, 한 층씩 자라는 그림이 아니라 매번 통째로 새 트리가 생긴다.

바깥 `SubPipeline` 의 원점은 투사체가 맞힌 적, 안쪽의 원점은 잎이다. `Link(Origin)` 이 그 잎을 부모로 삼는다.

이미 이어진 쌍은 `LinkHolder.Connect` 가 **수명만 갱신**하고 새 선을 긋지 않는다.

**부모-자식 방향은 선이 알려준다.** 선의 0번 점이 항상 부모다 — `LineRenderer` 의 Width Curve 를 `0.25 → 0.08` 로, Color Gradient 를 진한색 → 옅은색으로 두면 코드 없이 뿌리와 끝이 구분된다.

> 시트 `depth` 를 `Chain.maxDepthOverride` 와 `Link.hopsOverride` 가 함께 본다.
> 가지 길이와 피해 전이 홉을 따로 잡고 싶으면 둘 중 하나를 모듈에 직접 넣을 것.

**`maxChildren` 은 `Nearest` 모드 전용이다.**

```
0 (제한 없음)   뒤에 오는 노드가 전부 가까운 하나에 몰려 붙는다  →  부챗살
2 (이진 트리)   가까운 자리가 차면 바깥 노드에 붙는다            →  가지가 길게 뻗는다
```

제한이 없으면 "가장 가까운 것에 붙는다"가 곧 "제일 안쪽 노드에 다 붙는다"가 돼서 트리로 안 보인다. **2로 두면 진짜 이진 트리가 되고, 가지가 뻗는 모양이 눈에 들어온다.**

**`depth` 의 의미가 둘에서 다르다.**

```
Graph   모든 노드가 서로 이웃  →  홉이 1이든 3이든 결과가 비슷하다.  올려도 낭비
Tree    경로가 하나뿐          →  홉이 가지를 따라 얼마나 뻗는지를 정한다
```

`isPercent` 를 켜두면 홉마다 자연 감쇠한다 — 0.4면 40% → 16% → 6.4%. 뿌리에서 멀어질수록 옅어지는 그림이 공짜로 나오므로 Tree 는 비율 모드가 잘 맞는다.

| 튜닝 | Graph | Tree |
|---|---|---|
| `targetLimit` | 8~10 (간선이 제곱으로 는다) | 15~20 (간선이 N−1개라 여유롭다) |
| `depth` | 1~2 | 3~5 |
| `maxChildren` | 안 쓰임 | 2 |

> ⚠ **Graph 는 간선이 제곱으로 는다.** 노드 10개면 45줄, 15개면 105줄이다.
> 선 오브젝트가 그만큼 생기므로 `targetLimit` 을 8~10 안쪽으로 잡을 것.

**노드 수를 정하는 곳이 둘이다.**

```
SearchPoolTargeting.targetLimit   ← 여기가 0이면
시트 수량(count)                    ← 이 값이 노드 수가 된다
```

표식이 20개 붙어 있어도 `count` 가 3이면 **3개만 집는다.** 자료구조 계열은 노드 수가 곧 위력이라, 시트의 다른 용도(투사체 수)와 충돌하면 `targetLimit` 에 직접 넣는 편이 안전하다.

**노드당 간선 상한도 있다** — `LinkEffect.maxLinksPerNode`(기본 12). 넘치면 **가장 먼 간선부터** 밀려난다. 오래된 것부터 버리면 방금 만든 국소 구조가 먼저 깨져서 새로 이을 때마다 모양이 흐트러진다.
...
**전이는 피해를 적용하기 *전*에 흘려보낸다.**

```
1~4  피해량 확정 (표식 보정 · 크리티컬)
 5   링크 전이        ← 여기
 6   TakeDamage
 7   숫자 표시
```

적용 뒤에 전이하면, 그 피해로 대상이 죽는 순간 `OnDisable` 이 간선을 전부 끊어서 **센 공격일수록 전이가 안 되는** 거꾸로가 된다. 전이량은 4단계까지 이미 확정돼 있으므로 순서를 앞당겨도 값은 같다.
...
`Consume` 은 **모든 전달의 `Execute` 가 반환된 뒤** 불린다. 그런데 반환되는 의미가 전달마다 다르다.

```
Instant · Area · Line     판정 + 효과까지 다 끝내고 반환   →  쿨타임이 마지막에 돈다
Projectile · Radial       투사체를 생성만 하고 반환        →  쿨타임이 먼저 돈다
```

전달을 여러 개 넣으면 섞인다 — 즉발의 효과는 이미 끝났고 투사체의 효과는 아직인데 쿨타임이 돌기 시작한다. 쿨타임은 "다음 발동까지의 간격"이지 "효과가 끝날 때까지"가 아니므로 이것이 정상이다.

**효과는 적중 1회마다 목록 순서대로** 동기 실행된다. 적중이 세 번이면 목록을 세 번 돈다.

### 늦게 도착한 발동

투사체가 날아가는 동안 다음 발동이 시작될 수 있다. 그래서 **발동 A의 적중이 발동 B 이후에 도착**하는 일이 생긴다.

발동 단위 상태를 쓰는 모듈은 이걸 막아야 한다. `FiringId` 는 단조 증가하므로 비교로 가른다.

```csharp
if (ctx.FiringId < state.firingId) return;   // 늦게 온 지난 발동
```

| 모듈 | 늦게 왔을 때 |
|---|---|
| `Search` | 표식은 붙이되 **지난 표식 해제는 건너뛴다** — 그 공격도 유효했다 |
| `Link` (Nearest) | 건너뛴다 — 지금 쌓는 트리에 섞으면 엉뚱하게 이어진다 |
| `Link` (Origin) | 그대로 진행 — 발동 단위 상태를 안 쓴다 |

> 지금은 BFS 쿨타임 4초 · 비행 0.6초라 겹치지 않지만, **쿨타임을 낮추거나 속도를 낮추면 즉시 드러난다.**
> 지연 효과를 도입하면 이것이 예외가 아니라 일상이 되므로, 새 모듈에 발동 단위 상태를 둘 때는 반드시 같은 가드를 걸 것.
...
| `TreeFrontier` | 적 | **내 트리의 잎만** | `targetLimit` |
...
### 인스펙터가 시트 연결을 직접 보여준다

```
Projectile Count   [ 0 ] × [ 1 ]   ← 수량
Pierce             [ 0 ] × [ 2 ]   ← 관통력
Blast Radius       [ 0 ] × [ 1 ]   ← 효과범위
```

`[Sheet("수량")]` 을 붙이면 라벨 뒤에 **어느 시트 칸에서 오는지** 항상 보인다. 툴팁을 열지 않아도 읽힌다.

### 수량·관통도 배수를 준다

```
0 × 1     시트 그대로
0 × 2     시트의 두 배    ← 레벨업을 따라간다
0 × 0.5   절반
5 × 1     고정 5
```

전달을 여러 개 넣고 **각자 배수만 달리 주면** 서로 다른 발수·관통을 가지면서 **둘 다 레벨업을 따라간다.** 전에는 모듈에 숫자를 박으면 고정값이 되어 성장을 포기해야 했다.

정수라 반올림하되 0으로는 안 떨어뜨린다 — `0.4발`이 `0발`이 되면 조용히 사라지기 때문.

### 자주 안 만지는 칸은 "세부" 로 접힌다

`[Detail]` 이 붙은 칸은 인스펙터 아래쪽 접이 안으로 들어간다. **지우지 않는 이유** — 쓸 일이 드물 뿐 필요할 때는 반드시 있어야 하는 값들이다.

> 두 어트리뷰트 모두 **PropertyDrawer 가 아니다.** 어트리뷰트 드로어는 타입 드로어(`Scalable` 등)를 밀어내서 조립 버튼이 사라지는 사고가 난다. `AugmentModuleDrawer` 가 리플렉션으로 읽어 라벨과 배치만 바꾼다.
