...
public class AugmentData : ScriptableObject
{
    ...
    [Tooltip("이 증강의 레벨 수치를 뿌리의 보정으로 합칠지.\n\n" +
             "켜면 — 스탯 변경형. 내 수치가 뿌리 수치에 얹혀 뿌리가 통째로 세진다.\n" +
             "끄면 — 내 수치는 내 효과만 쓴다. 뿌리는 전혀 안 변한다.\n\n" +
             "★ Extension Slot 에 꽂히면서 자기 수치로 계산하는 증강은 반드시 끌 것.\n" +
             "   예) Bash:kill 의 효과피해 0.3 을 켜두면 뿌리 Bash 의 효과피해에도 +30% 가 붙어\n" +
             "       연쇄 단계 보너스까지 같이 세진다 — 마지막 한 방만 원했는데.")]
    public bool mergeStatsIntoRoot = true;

    [Tooltip("위 합치기를 켰을 때, 실수 칸을 어떻게 얹을지.\n\n" +
             "켜면 비율이다 — 피해량 0.2 는 뿌리 피해 +20%.\n" +
