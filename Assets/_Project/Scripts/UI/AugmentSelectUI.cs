...
public class AugmentSelectUI : MonoBehaviour
{
    ...
    void OnCardClicked(Card card)
    {
        ...
        AugmentRunner runner = augmentManager.Grant(card.Data);
        levelSystem.ConsumePendingLevelUp();

        if (LogManager.Instance != null && runner != null)
            LogManager.Instance.Skill(
                $"AUGMENT LOADED: {card.Data.displayName} Lv.{runner.Instance.Level}");
    ...
    List<AugmentData> CollectCandidates()
    {
        ...
        {
            if (data == null || data.levelStats == null || data.levelStats.Length == 0)
                continue;
